...
public:
   ...
   Punto2D Centro(){
      double cantidad_a_sumar = longitud / 2.0;
		double abcisa = esquina_abscisa + cantidad_a_sumar;
		double ordenada = esquina_ordenada + cantidad_a_sumar;
		
		Punto2D centro(abcisa, ordenada);
		return centro;      
   }

   Circunferencia CircunferenciaInscrita(){
      Punto2D centro(Centro());
      double abcisa = centro.Abscisa();
      double ordenada = centro.Ordenada();
      double radio = longitud / 2.0;
      
      Circunferencia inscrita(abcisa, ordenada, radio);
      return inscrita;
   }
   ...
   Circunferencia CircunferenciaCircunscrita(){
      ...
      double abcisa = centro.Abscisa();
      double ordenada = centro.Ordenada();
      SegmentoDirigido radio(abcisa, ordenada, esquina_abscisa, esquina_ordenada);
      
      Circunferencia Circunscrita(abcisa, ordenada, radio.Longitud());
      return Circunscrita;
   }
   ...
   bool Contiene(Cuadrado otro){
      if(otro.AbscisaEsquina() >= esquina_abscisa &&
      	otro.AbscisaEsquina() + otro.Longitud() <= esquina_abscisa + longitud &&
      	otro.OrdenadaEsquina() >= esquina_ordenada &&
      	otro.OrdenadaEsquina() + otro.Longitud() <= esquina_ordenada + longitud)
      		return true;
      else
      	return false;
   }
